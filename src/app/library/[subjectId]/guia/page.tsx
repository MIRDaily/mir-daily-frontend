...
export default async function LibraryGuidePage({ params }: LibraryGuidePageProps) {
  ...
  const toc: GuideTocItem[] = [
    { id: 'conclusiones', label: `Qué nos dice el ${guide.lastExam}`, icon: 'insights' },
    { id: 'tendencia', label: 'Preguntas por año', icon: 'bar_chart' },
    { id: 'tendencias', label: 'Qué sube y qué baja', icon: 'swap_vert' },
    { id: 'prioridades', label: 'Mapa de prioridades', icon: 'format_list_numbered' },
    { id: 'evolucion', label: 'Evolución tema a tema', icon: 'grid_on' },
    { id: 'rentabilidad', label: 'Esfuerzo y recompensa', icon: 'scatter_plot' },
    ...
    {
      id: 'ultimo-mir',
      label: `${guide.lastExam} comentado`,
      icon: 'quiz',
        ...
        <div className="flex min-w-0 flex-col gap-10">
          ...
          <section id="ultimo-mir" className="flex scroll-mt-24 flex-col gap-8">
            ...
            {QUESTION_GROUPS.map((group) => {
              ...
              return (
                <div key={group.kind} id={`ultimo-mir-${group.kind}`} className="flex scroll-mt-24 flex-col gap-4">
